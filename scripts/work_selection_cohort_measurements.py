...
_PROBE_FIXED_ZONE_CAST_LIFECYCLES = (
    "fixed-zone-cast-lifecycles-existing-owner-v1"
)
...
_PROBE_FIXED_PUBLIC_DAMAGE_PREDICATES = (
    "fixed-public-damage-predicate-existing-owner-v1"
)
_PROBE_FIXED_PUBLIC_ALTERNATIVE_COSTS = (
...
_PROBE_FIXED_CONSTRAINED_MANA = (
    "fixed-constrained-mana-existing-owner-v1"
)
_PROBE_DYNAMIC_SELF_ENTRY_COUNTER = (
    "dynamic-self-entry-counter-existing-owner-v1"
)
...
_FIXED_MANA_MADNESS_FAMILIES = frozenset(
    {
        _CONTINUOUS_LAYER_FAMILY,
        "effect_clause:unparsed-madness-0",
        "effect_clause:unparsed-madness-1-b",
        "effect_clause:unparsed-madness-1-r",
        "effect_clause:unparsed-madness-1-u",
        "effect_clause:unparsed-madness-2-b",
        "effect_clause:unparsed-madness-3-b",
        "effect_clause:unparsed-madness-3-r",
        "effect_clause:unparsed-madness-3-u",
        "effect_clause:unparsed-madness-b",
        "effect_clause:unparsed-madness-b-r",
        "effect_clause:unparsed-madness-r",
        "effect_clause:unparsed-madness-u",
        "effect_clause:unparsed-madness-w",
    }
)
...
def _fixed_zone_cast_lifecycle_measurement(
    *,
    frontier: Mapping[str, Any],
    bundle_id: str,
    probe_id: str,
    ...
    """Measure one closed five-mechanic zone-cast lifecycle cohort."""

    selected_kinds = {"escape", "foretell", "jump-start", "plot", "rebound"}
    registry = load_default_capability_registry()
    matched_cards: dict[str, int] = {}
    exact_ability_gain = 0
    complete_cards = 0
    ...
    for card in frontier.get("cards", []):
        ...
        for ability in card.get("abilities", ()):
            if ability.get("status") == "exact":
                continue
            spec = fixed_cast_lifecycle_spec(_source_line(record, ability))
            if spec is not None and spec.kind.value in selected_kinds:
                lifecycle_abilities.append(ability)
        if not lifecycle_abilities:
            continue
        compiled = compile_oracle_card(
            record,
            capability_registry=registry,
        ...
        if not represented:
            unsupported_grammar_cards.add(oracle_id)
            continue
        if len(represented) != len(lifecycle_abilities):
            unsupported_grammar_cards.add(oracle_id)
        exact_ability_gain += len(represented)
        matched_cards[oracle_id] = len(represented)
        remaining = [
...
def _fixed_source_characteristic_measurement(
    *,
    frontier: Mapping[str, Any],
    bundle_id: str,
    probe_id: str,
    ...
    for card in frontier.get("cards", []):
        ...
        candidate_abilities = [
            ...
            and family_ids.intersection(member_ids)
            and family_ids <= member_ids
        ]
        ...
        if not represented:
            unsupported_grammar_cards.add(oracle_id)
            continue
        if len(represented) != len(candidate_abilities):
            unsupported_grammar_cards.add(oracle_id)
        exact_ability_gain += len(represented)
        matched_cards[oracle_id] = len(represented)
        remaining = [
...
def _fixed_public_damage_predicate_measurement(
    *,
    frontier: Mapping[str, Any],
    bundle_id: str,
    probe_id: str,
    ...
    """Measure newly exact fixed damage over public typed predicates."""
    ...
    for card in frontier.get("cards", []):
        ...
        if not blocker_set or not blocker_set.issubset(member_ids):
            continue
        oracle_id = str(card.get("oracle_id") or "")
        record = cards_by_oracle_id.get(oracle_id)
        if record is None:
        ...
        represented = [
            ...
            if previous.get(node.node_id, {}).get("status") != "exact"
        ]
        if not represented:
            continue
        matched_cards[oracle_id] = len(represented)
...
def _fixed_public_alternative_cost_measurement(
    *,
    frontier: Mapping[str, Any],
    bundle_id: str,
    probe_id: str,
    ...
    """Measure only newly exact fixed public alternative-cost handlers."""
    ...
    for card in frontier.get("cards", []):
        ...
        oracle_text = str(record.oracle_text).casefold()
        if (
            "rather than pay this spell's mana cost" not in oracle_text
            and not any(
                f"{keyword} " in oracle_text
                for keyword in ("freerunning", "prowl", "spectacle", "surge")
            )
        ):
            continue
        compiled = compile_oracle_card(
        ...
        represented = [
            ...
            for node in face.nodes
            if node.exact
            and FIXED_PUBLIC_ALTERNATIVE_COST_CAPABILITY
            in node.capability_dependencies
            and previous.get(node.node_id, {}).get("status") != "exact"
        ]
        if not represented:
...
def _fixed_restrictive_library_search_measurement(
    *,
    frontier: Mapping[str, Any],
    bundle_id: str,
    probe_id: str,
    ...
    """Measure newly exact searches through the existing hidden-search owner."""
    ...
    for card in frontier.get("cards", []):
        ...
        if "search your library" not in str(record.oracle_text).casefold():
            continue
        compiled = compile_oracle_card(
            record,
            capability_registry=registry,
...
def _fixed_constrained_mana_measurement(
    *,
    frontier: Mapping[str, Any],
    bundle_id: str,
    probe_id: str,
    ...
    """Measure newly exact fixed mana nodes through shared typed constraints."""

    registry = load_default_capability_registry()
    constrained_capabilities = {
        "activation.condition.public_query",
        "activation.usage.once_per_turn",
    ...
    for card in frontier.get("cards", []):
        ...
        represented = [
            node
            for node in compiled_exact
            if constrained_capabilities.intersection(
...
def _dynamic_self_entry_counter_measurement(
    *,
    frontier: Mapping[str, Any],
    bundle_id: str,
    probe_id: str,
    ...
    """Measure newly exact dynamic self-entry counter replacements."""

    registry = load_default_capability_registry()
    capability_id = "counter.producer.dynamic_self_entry"
    broad_cards: set[str] = set()
    matched_cards: dict[str, int] = {}
    exact_ability_gain = 0
    complete_cards = 0
    ...
    for card in frontier.get("cards", []):
        ...
        candidates = [
            ...
            and " enters with "
            in _source_line(record, ability).casefold()
        ]
        if not candidates:
            continue
        broad_cards.add(oracle_id)
        ...
        represented = [
            node
            for node in compiled_exact
            if capability_id in node.capability_dependencies
            and previous.get(node.node_id, {}).get("status") != "exact"
        ]
        if not represented:
            continue
...
def _fixed_public_declaration_condition_measurement(
    *,
    frontier: Mapping[str, Any],
    bundle_id: str,
    probe_id: str,
    ...
    """Measure newly exact public conditional declaration programs."""
    ...
    for card in frontier.get("cards", []):
        ...
        represented = [
            nodes[str(ability.get("ability_id") or "")]
            for ability in candidates
            if str(ability.get("ability_id") or "") in nodes
            and nodes[str(ability.get("ability_id") or "")].exact
            and "combat.declaration.typed_components"
            in nodes[str(ability.get("ability_id") or "")]
    ...
    return {
        ...
        "decision": (
            "bounded_executable"
            if reaches_floor and complete_cards > 0
            else "retired_below_harvest_floor"
        ),
        "grants_gameplay_trust": False,
        "candidate_accounting": {
            "affected_oracle_carriers": sum(matched_cards.values()),
            "existing_exact_sibling_nodes": existing_exact_siblings,
            "remaining_residual_sibling_nodes": remaining_residual_siblings,
            "trusted_program_transitions": complete_cards,
            "unresolved_program_transitions": (
...
def _measurement(
    *,
    ...
    if probe_id == _PROBE_FIXED_ZONE_CAST_LIFECYCLES:
        return _fixed_zone_cast_lifecycle_measurement(
            frontier=frontier,
            bundle_id=bundle_id,
            probe_id=probe_id,
            cards_by_oracle_id=cards_by_oracle_id,
    ...
    if probe_id == _PROBE_FIXED_SOURCE_CHARACTERISTICS:
        return _fixed_source_characteristic_measurement(
            frontier=frontier,
            bundle_id=bundle_id,
            probe_id=probe_id,
    ...
    if probe_id == _PROBE_FIXED_PUBLIC_DAMAGE_PREDICATES:
        return _fixed_public_damage_predicate_measurement(
            frontier=frontier,
            bundle_id=bundle_id,
            probe_id=probe_id,
            member_ids={str(value) for value in bundle["member_family_ids"]},
    ...
    if probe_id == _PROBE_FIXED_PUBLIC_ALTERNATIVE_COSTS:
        return _fixed_public_alternative_cost_measurement(
            frontier=frontier,
            bundle_id=bundle_id,
            probe_id=probe_id,
            cards_by_oracle_id=cards_by_oracle_id,
    ...
    if probe_id == _PROBE_FIXED_RESTRICTIVE_LIBRARY_SEARCHES:
        return _fixed_restrictive_library_search_measurement(
            frontier=frontier,
            bundle_id=bundle_id,
            probe_id=probe_id,
            cards_by_oracle_id=cards_by_oracle_id,
    ...
    if probe_id == _PROBE_FIXED_CONSTRAINED_MANA:
        return _fixed_constrained_mana_measurement(
            frontier=frontier,
            bundle_id=bundle_id,
            probe_id=probe_id,
            member_ids={str(value) for value in bundle["member_family_ids"]},
    ...
    if probe_id == _PROBE_DYNAMIC_SELF_ENTRY_COUNTER:
        return _dynamic_self_entry_counter_measurement(
            frontier=frontier,
            bundle_id=bundle_id,
            probe_id=probe_id,
            member_ids={str(value) for value in bundle["member_family_ids"]},
    ...
    if probe_id == _PROBE_FIXED_PUBLIC_DECLARATION_CONDITION:
        return _fixed_public_declaration_condition_measurement(
            frontier=frontier,
            bundle_id=bundle_id,
            probe_id=probe_id,
            member_ids={str(value) for value in bundle["member_family_ids"]},
    ...
    if probe_id == _PROBE_FIXED_PUBLIC_CHARACTERISTIC_SET:
        return _fixed_public_characteristic_set_measurement(
            frontier=frontier,
            bundle_id=bundle_id,
            probe_id=probe_id,
            member_ids=member_ids,
...
def _fixed_mana_madness_measurement(
    *,
    frontier: Mapping[str, Any],
    bundle_id: str,
    probe_id: str,
    ...
    """Measure the two-ability fixed-mana Madness lifecycle."""
    ...
    for card in frontier.get("cards", []):
        ...
        madness_abilities = [
            ability
            for ability in residual_abilities
            if _source_line(record, ability).strip().casefold().startswith(
                "madness"
            )
        ]
        candidates = [
            ability
            for ability in madness_abilities
            if _FIXED_MANA_MADNESS_FAMILIES.intersection(
                {
                str(value)
                for value in ability.get("blockers", {}).get(
                    "canonical_family_ids", ()
        ...
        if madness_abilities and not candidates:
            unsupported_grammar_cards.add(oracle_id)
        if not candidates:
            continue
        compiled = compile_oracle_card(
        ...
        for ability in candidates:
            ...
            nodes = (
                [
                    ...
                    if node.span.line == source_line
                    and node.exact
                    and node.template_id in templates
                ]
                if face is not None
                else []
            )
            if len(nodes) == 2 and {node.template_id for node in nodes} == templates:
                represented += 1
        if not represented:
            unsupported_grammar_cards.add(oracle_id)
            continue
...
_FIXED_PUBLIC_CHARACTERISTIC_SCOPE = re.compile(
    r"(?:All creatures|Creatures your opponents control|"
    ...
    r"Each other attacking creature) (?:get|gets|gain|gains) .+ "
    r"until end of turn\.?(?:\s*\([^)]*\))?$",
    re.IGNORECASE,
)


def _contains_fixed_public_characteristic_effect(value: Any) -> bool:
    if isinstance(value, Mapping):
        if (
            value.get("op")
            == "modify_all_matching_permanents_until_end_of_turn"
            and type(value.get("power")) is int
            and type(value.get("toughness")) is int
            and isinstance(value.get("predicate"), Mapping)
        ):
            return True
        return any(
            _contains_fixed_public_characteristic_effect(child)
            for child in value.values()
        )
    if isinstance(value, (list, tuple)):
        return any(
...
def _fixed_public_characteristic_set_measurement(
    *,
    frontier: Mapping[str, Any],
    bundle_id: str,
    probe_id: str,
    ...
    """Measure exact public creature-set modifiers through the shared owner."""
    ...
    for card in frontier.get("cards", []):
        ...
        candidates = [
            ...
            for ability in card.get("abilities", [])
            if ability.get("status") != "exact"
            and _FIXED_PUBLIC_CHARACTERISTIC_SCOPE.search(
        ...
        card_matches = sum(
            node is not None
            and node.exact
            and _contains_fixed_public_characteristic_effect(node.to_dict())
            for ability in candidates
            for node in (nodes.get(str(ability.get("ability_id") or "")),)
        )
        if not card_matches:
...
def _fixed_static_declaration_composition_measurement(
    *,
    frontier: Mapping[str, Any],
    bundle_id: str,
    probe_id: str,
    ...
    """Measure exact static declaration composition after integration."""
    ...
    for card in frontier.get("cards", []):
        ...
        for ability in card.get("abilities", []):
            ...
            line = _source_line(record, ability)
            if (
                fixed_static_declaration_grant_handler(
                    line,
                    source_name=source_name,
